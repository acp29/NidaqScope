# Add nidaq_scope directory to PYTHONPATH in windows
...
qtCreatorFile = r".\nidaq_scope_gui.ui" #Path for ui file
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    ...
    def pulse_graph(self): # Show pulse graph
        ...
        for i in range(0,int(settings['postpulse']*1000)):
            wave.append(settings['holding'])	
        graph= pg.plot(wave)
        graph.setLabel('left', 'Voltage (V)' )
        graph.setLabel('bottom', 'Time (ms)',)
		
    def pulse_window(self):
        self.window3 = QtGui.QDialog()
        ...
        self.window3.show()
        self.ui3.down_spinner_1.setValue(np.float(settings['prepulse']))
        self.ui3.amplitude_spinner.setValue(np.float(settings['pulse_amp']))		
        self.ui3.pulse_length_spinner.setValue(np.float(settings['pulse_length']))		
        self.ui3.down_spinner_2.setValue(np.float(settings['postpulse']))		
        self.ui3.holding_voltage_spinner.setValue(np.float(settings['holding']))
        self.ui3.commander_scale_spinner.setValue(np.float(settings['commander_scale']))		
